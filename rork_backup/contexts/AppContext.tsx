...
export const [AppProvider, useApp] = createContextHook(() => {
  ...
  const maxContacts = useMemo(() => {
    return subscriptionLevel === 'working' ? Infinity : 20;
  }, [subscriptionLevel]);

  const canAddContact = useMemo(() => {
    return dossiers.length < (subscriptionLevel === 'working' ? Infinity : 20);
  }, [dossiers.length, subscriptionLevel]);
  ...
  const createBackup = useCallback(async () => {
    ...
    try {
      ...
      console.error('[AppContext] createBackup failed', error);
      return false;
    }
  ...
  const restoreBackup = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.error('[AppContext] restoreBackup failed to read file', { fileUri, error });
        return false;
      }
    } catch (error) {
      console.error('Failed to restore backup:', error);
  ...
  const isBootstrapped = phoneQuery.data !== undefined;
