...
import type { ContactDossier, StrategicGoal, GoalDirection } from '@/types';
...
const STORAGE_KEYS = {
  ...
  GOALS: 'strategic_goals_v1',
  GOAL_DIRECTIONS: 'goal_directions_v1',
...
export const [AppProvider, useApp] = createContextHook(() => {
  ...
  const [goalDirections, setGoalDirections] = useState<GoalDirection[]>([]);
  ...
  const goalDirectionsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        return parsed.length > 0 ? parsed : DEFAULT_GOAL_DIRECTIONS;
      } catch {
  ...
  useEffect(() => {
    if (goalDirectionsQuery.data) {
      setGoalDirections(goalDirectionsQuery.data);
    }
  }, [goalDirectionsQuery.data]);
  ...
  const addDossier = useCallback((dossier: ContactDossier): { ok: true } | { ok: false; error: 'DUPLICATE'; existingId: string; existingName: string } => {
    ...
    void saveAppDataToServer({ dossiers: updated, sectors, powerGroupings });
    return { ok: true };
  ...
  const addSector = useCallback((sector: string) => {
    ...
    void saveAppDataToServer({ dossiers, sectors: updatedSectors, powerGroupings });
  }, [dossiers, saveAppDataToServer, sectors, powerGroupings]);
  ...
  const removeSector = useCallback((sector: string) => {
    ...
    void saveAppDataToServer({ dossiers: updatedDossiers, sectors: updatedSectors, powerGroupings });
    return { error: false };
  }, [dossiers, saveAppDataToServer, sectors, powerGroupings]);
  ...
  const addPowerGrouping = useCallback((groupName: string) => {
    ...
    void saveAppDataToServer({ dossiers, sectors, powerGroupings: updated });
  }, [dossiers, saveAppDataToServer, sectors, powerGroupings]);
  ...
  const removePowerGrouping = useCallback((groupName: string) => {
    ...
    void saveAppDataToServer({ dossiers: updatedDossiers, sectors, powerGroupings: updated });
  }, [dossiers, saveAppDataToServer, sectors, powerGroupings]);
  ...
  const addGoal = useCallback((goal: StrategicGoal) => {
    ...
    void persistGoals(updated);
  }, [goals, persistGoals]);

  const updateGoal = useCallback((id: string, updates: Partial<StrategicGoal>) => {
  ...
  const addGoalDirection = useCallback((direction: GoalDirection) => {
    const updated = [...goalDirections, direction];
    void persistGoalDirections(updated);
  }, [goalDirections, persistGoalDirections]);

  const updateGoalDirection = useCallback((id: string, updates: Partial<GoalDirection>) => {
    const updated = goalDirections.map(d => d.id === id ? { ...d, ...updates } : d);
    void persistGoalDirections(updated);
  }, [goalDirections, persistGoalDirections]);

  const deleteGoalDirection = useCallback((id: string) => {
    const updated = goalDirections.filter(d => d.id !== id);
    void persistGoalDirections(updated);
    const updatedGoals = goals.map(g => g.directionId === id ? { ...g, directionId: undefined } : g);
    void persistGoals(updatedGoals);
  }, [goalDirections, persistGoalDirections, goals, persistGoals]);

  const reorderGoalDirections = useCallback((reordered: GoalDirection[]) => {
    void persistGoalDirections(reordered);
  }, [persistGoalDirections]);
