...
export default function NetworkScreen({ onOpenDossier }: NetworkScreenProps) {
  const { dossiers, sectors, theme, powerGroupings, t, currentTheme, goals, goalDirections } = useApp();
  ...
  const [filterDirectionIds, setFilterDirectionIds] = useState<string[]>([]);
  ...
  const goalsForSelectedDirections = useMemo(
    () => goals.filter(g => g.directionId && filterDirectionIds.includes(g.directionId)),
    [goals, filterDirectionIds]
  );

  const handleToggleDirection = useCallback((dirId: string) => {
    setFilterDirectionIds(prev => {
      const isSelected = prev.includes(dirId);
      if (isSelected) {
        const nextDirs = prev.filter(id => id !== dirId);
        const removedGoalIds = goals.filter(g => g.directionId === dirId).map(g => g.id);
        setFilterGoalIds(prevGoals => prevGoals.filter(id => !removedGoalIds.includes(id)));
        return nextDirs;
      } else {
        const addedGoalIds = goals.filter(g => g.directionId === dirId).map(g => g.id);
        setFilterGoalIds(prevGoals => [...new Set([...prevGoals, ...addedGoalIds])]);
        return [...prev, dirId];
      }
    });
  }, [goals]);
  ...
  const handleResetGoalFilter = useCallback(() => {
    setFilterDirectionIds([]);
    setFilterGoalIds([]);
  }, []);
  ...
  }, [dossiers, filterSectors, filterGoalIds, filterPowerGroupings, goals]);
            ...
            <Svg width={canvasWidth} height={canvasHeight}>
              {ringData
                ...
                .map(({ circle, innerRadius, outerRadius, segments, sectorCircleCounts }) => {
                  const angleSource =
                    sectorData.sectorAngles; // eslint-disable-line
              ...
              {sectorData.sectorAngles.map(
                ({ sector, startAngle, endAngle, count }) => {
                  ...
                  return (
                    <G key={`sector-${sector}`}>
                      ...
                      <SvgText
                        ...
                        fontWeight="700"
                        opacity="0.9"
                        transform={`rotate(${(midAngle * 180) / Math.PI + 90} ${centerX + labelRadius * Math.cos(midAngle)} ${centerY + labelRadius * Math.sin(midAngle)})`}
                      >
    ...
    <View style={styles.background}>
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View>
          ...
          <Text style={styles.filterLabel}>{(t.network as any).goalFilter || 'ЦЕЛЬ'}</Text>
          <WheelHorizontalScroll contentContainerStyle={styles.filters} testID="network-filter-goal-directions-scroll">
            <TouchableOpacity
              style={[
                styles.filterButton,
                filterDirectionIds.length === 0 && styles.filterButtonActive,
              ]}
              onPress={handleResetGoalFilter}
              activeOpacity={0.7}
            >
              <Text
                style={[
                  styles.filterText,
                  filterDirectionIds.length === 0 && styles.filterTextActive,
                ]}
              >
                {t.network.all}
              </Text>
            ...
            {sortedDirections.map((dir) => {
              const isSelected = filterDirectionIds.includes(dir.id);
              return (
                <TouchableOpacity
                  key={dir.id}
                  style={[
                  ...
                  onPress={() => handleToggleDirection(dir.id)}
                  activeOpacity={0.7}
                >
                  <Text
                    style={[
          ...
          {filterDirectionIds.length > 0 && goalsForSelectedDirections.length > 0 && (
            <WheelHorizontalScroll contentContainerStyle={styles.goalSubFilters} testID="network-filter-goals-scroll">
              {goalsForSelectedDirections.map((goal) => {
                const isSelected = filterGoalIds.includes(goal.id);
                return (
                  <TouchableOpacity
                    key={goal.id}
                    style={[
                      styles.goalSubFilterButton,
                      isSelected && styles.goalSubFilterButtonActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.goalSubFilterText,
                        isSelected && styles.goalSubFilterTextActive,
                      ]}
                      numberOfLines={1}
                    >
