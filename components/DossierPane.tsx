...
import type { ContactRelation, DiaryEntry, FunctionalCircle, ImportanceLevel, Sector } from '@/types';
...
export function DossierPane({ dossierId, initialEdit, onBack, onOpenNetwork }: DossierPaneProps) {
  const {
    dossiers,
    updateDossier,
    deleteDossier,
...
Результат:`;
  ...
  const handleRemoveSuzerain = useCallback(() => {
    ...
    const updatedRelations = editRelations.filter((r) => r.contactId !== oldSuzerainId);
    setEditRelations(updatedRelations);

    if (dossier) {
      updateDossier(dossierId, {
        relations: updatedRelations,
        powerGrouping: editPowerGroupName
          ? { groupName: editPowerGroupName, suzerainId: undefined, vassalIds: editVassalIds }
          : undefined,
      });
    }
      ...
      setTimeout(() => {
        ...
        if (suzerainDossier) {
          updateDossier(oldSuzerainId, {
            powerGrouping: suzerainDossier.powerGrouping
              ? {
                  ...suzerainDossier.powerGrouping,
                  vassalIds: suzerainDossier.powerGrouping.vassalIds.filter((vid) => vid !== dossierId),
                }
              : undefined,
            relations: suzerainDossier.relations.filter((r) => r.contactId !== dossierId),
          });
        }
      }, 100);
    }
  }, [dossier, dossiers, dossierId, editPowerGroupName, editRelations, editSuzerainId, editVassalIds, updateDossier]);
  ...
  const handleRemoveVassal = useCallback(
    (vassalId: string) => {
      const updatedVassalIds = editVassalIds.filter((vid) => vid !== vassalId);
      setEditVassalIds(updatedVassalIds);

      const updatedRelations = editRelations.filter((r) => r.contactId !== vassalId);
      setEditRelations(updatedRelations);
      ...
      if (dossier) {
        updateDossier(dossierId, {
          relations: updatedRelations,
          powerGrouping: editPowerGroupName
            ? { groupName: editPowerGroupName, suzerainId: editSuzerainId, vassalIds: updatedVassalIds }
            : undefined,
        });
      }
      ...
      setTimeout(() => {
        ...
        if (vassalDossier) {
          updateDossier(vassalId, {
            powerGrouping: vassalDossier.powerGrouping?.suzerainId === dossierId
              ? undefined
              : vassalDossier.powerGrouping,
            relations: vassalDossier.relations.filter((r) => r.contactId !== dossierId),
          });
        }
      }, 100);
    },
            ...
            <View style={styles.profileSection} testID="dossierProfileSection">
              ...
              {isEditing ? (
                ...
                <>
                  ...
                  {dossier.contact.company && <Text style={styles.company}>{dossier.contact.company}</Text>}
                  {dossier.contact.goal && <Text style={styles.goal}>{dossier.contact.goal}</Text>}
                </>
              )}
            </View>
                  ...
                  try {
                    ...
                    if (status === 'granted') {
                      ...
                      const existingContact = data.find((c) =>
                        c.phoneNumbers?.some((p) => dossier.contact.phoneNumbers.includes(p.number || ''))
                      );
                        ...
                        <View style={styles.editRelationHeader}>
                          ...
                          <TouchableOpacity
                            onPress={() => {
                              ...
                              const updatedRelations = editRelations.filter((_, i) => i !== idx);
                              setEditRelations(updatedRelations);
                              ...
                              if (removedRelation) {
                                setTimeout(() => {
                                  const targetDossier = dossiers.find(
                                    (x) => x.contact.id === removedRelation.contactId
                                  );
                                  if (targetDossier) {
                                    updateDossier(removedRelation.contactId, {
                                      relations: targetDossier.relations.filter(
